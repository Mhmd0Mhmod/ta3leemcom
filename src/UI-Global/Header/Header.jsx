  ...
  return (
    <nav className={'relative mb-3 ml-10 flex justify-between gap-4 font-almaria'}>
      ...
      <ul className={'flex items-center justify-between gap-5 text-3xl'}>
        <Logo />
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/home' ? 'active' : ''}`}>
          <Link to="/home">الرئيسية</Link>
        </li>
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/about' ? 'active' : ''}`}>
          <Link to="/about">عن</Link>
        </li>
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/services' ? 'active' : ''}`}>
          <Link to="/services">الخدمات</Link>
        </li>
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/instructions' ? 'active' : ''}`}>
          <Link to="/instructions">تعليمات</Link>
        </li>
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/subscriptions' ? 'active' : ''}`}>
          <Link to="/subscriptions">الاشتركات</Link>
        </li>
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/opinion' ? 'active' : ''}`}>
          <Link to="/opinion">رأيك</Link>
        </li>
        <li className={`whitespace-nowrap pb-2 duration-500 hover:text-[#0884A2] ${currentPath === '/contact-with-us' ? 'active' : ''}`}>
        ...
        <div className={'flex w-full items-center justify-between'}>
          <div className={'relative flex items-center gap-4'}>
            ...
            <button className={'flex items-center gap-[10px] text-2xl'} onClick={() => setOpenTabsList(!openTabsList)}>
              ...
              {role === 'teacher' && <DropDashBoardList className={`${openTabsList ? 'rotate-180' : ''} duration-200`} />}
            </button>
            {openTabsList && (
            ...
            {openList && (
              <ul className={'absolute left-2 top-16 z-40 flex flex-col gap-4 rounded border bg-white p-2'}>
                ...
                <li
                  ...
                  className={'flex w-48 cursor-pointer items-center gap-4 rounded p-1 text-xl duration-300 hover:bg-[#FFB2B3]'}
                >
                  <Logout />
