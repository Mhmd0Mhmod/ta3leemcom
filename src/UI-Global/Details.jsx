...
function Details({ className, summary, route, Icon, listIcon = true, listItems = [], tabName = [], param, opend, children }) {
  ...
  return (
    <div className="relative">
        ...
        {!showList && (
          <ul className={'flex flex-col gap-2'}>
            {listItems.map((item, index) => (
              <li key={index} onClick={() => handleTo(tabName[index])} className={`flex cursor-pointer items-center gap-2 rounded p-2.5 pr-7 text-[#757474] duration-300 hover:bg-[#b4d3e0] ${activeTab === tabName[index] ? 'active' : ''}`}>
      ...
      {showList && (
        <ul className={'absolute -left-[120px] top-0 z-10 min-w-[110px] rounded-[7px] bg-white text-[14px] shadow-lg'}>
          {listItems.map((item, index) => (
            <Fragment key={index}>
              <li onClick={() => handleTo(tabName[index])} className={`cursor-pointer rounded p-1 pl-4 duration-300 hover:bg-[#D7D7D7] ${activeTab === tabName[index] ? 'active' : ''}`}>
                <span>{item.name}</span>
              </li>
            </Fragment>
          ))}
