    ...
    return (
        <div className="relative">
            <details open={opend ? '' : false} onClick={handleOpen}>
                <summary
                    className={'mb-2 flex cursor-pointer items-center gap-[18px] rounded p-2.5 duration-300 hover:bg-[#b4d3e0] ' + className}>
                ...
                {!showList && (
                    <ul className={'flex flex-col gap-2'}>
                        {listItems.map((item, index) => (
                            <li key={index} onClick={() => handleTo(tabName[index])}
                                className={`flex cursor-pointer items-center gap-2 rounded p-2.5 pr-7 text-[#757474] duration-300 hover:bg-[#b4d3e0] ${activeTab === tabName[index] ? 'active' : ''}`}>
                ...
                <ul className={'absolute -left-[120px] top-0 z-10 w-[110px] rounded-[7px] bg-white text-[14px] shadow-lg'}>
                    {listItems.map((item, index) => (
                        <Fragment key={index}>
                            <li
                                onClick={() => {
                                    setSearchParams({tab: item.id, [param]: tabName[index]});
                                    setShowList(false);
                                }}
