...
function TeacherDashboard() {
    ...
    return (<div className={"flex dashboard gap-5 font-cairo"}>
        <div className={`p-2.5 bg-gray-100 rounded ${opened ? "w-[320px]" : "w-fit"}  self-start`}>
            <div className={"flex justify-end"}>
                <Icon src={menu} className={`w-9 ${opened ? "" : "rotate-180"}`}
            ...
            <ul className={"flex gap-2 flex-col font-cairo-bold"}>
                ...
                <li onClick={() => handleTabClick("addGroup")}
                    className={`rounded p-2.5 flex items-center gap-1 cursor-pointer ${activeTab === "addGroup" ? "active" : ""}`}>
                    <Icon src={group}/>
                    <span className={`${!opened ? "hidden" : ""}`}>اضافة مجموعة</span>
                </li>
                <li className={`rounded p-2.5  cursor-pointer ${activeTab === "addTest" ? "active" : ""}`}>
                    <Details summary={"اضافة اختبار"} icon={TestIcon}
                             listItems={["اونلاين", "اوفلاين"]} tabName={["online", "offline"]} param={"test"}
                             opend={opened}/>
                </li>
                <li className={`rounded p-2.5 cursor-pointer ${activeTab === "StudyLevels" ? "active" : ""}`}>
                    <Details summary={"المراحل الدراسية"} icon={graduted} opend={opened}
                             ...
                             tabName={["primary", "middle", "high"]} param={"level"}/>
                </li>
                <li onClick={() => handleTabClick("meeting")}
                    className={`rounded p-2.5 flex items-center gap-1 cursor-pointer ${activeTab === "meeting" ? "active" : ""}`}>
                    <Icon src={meeting}/>
