    ...
    <div className={"flex dashboard gap-5 font-cairo mb-4   "}>
      <div className={`p-2.5 bg-gray-100 rounded ${opened ? "w-[320px]" : "w-fit"} self-start`}>
        ...
        <ul className={"flex gap-2 flex-col font-cairo text-xl mt-2 "}>
          <li onClick={() => handleTabClick("addStudent")} className={`rounded p-2.5 flex items-center gap-[18px] cursor-pointer hover:bg-[#b4d3e0] duration-300 h ${activeTab === "addStudent" ? "active" : ""}`}>
            <Profile />
            <span className={`${!opened ? "hidden" : ""}`}>اضافة طالب</span>
          </li>
          <li onClick={() => handleTabClick("addGroup")} className={`rounded p-2.5 flex items-center gap-[18px] cursor-pointer hover:bg-[#b4d3e0] duration-300 ${activeTab === "addGroup" ? "active" : ""}`}>
            <Group />
      ...
      <div className={" p-10 bg-gray-100 self-stretch flex-grow rounded relative"}>
        ...
        {showScrollTop && (
          <button onClick={scrollToTop} className="absolute left-12 bottom-12 hover:-translate-y-1 transition-all duration-300">
            <ScrollTop />
          </button>
        )}
      </div>
