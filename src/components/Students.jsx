...
function Students() {
    ...
    useEffect(() => {
        if (search) {
            setStudents(FakeGroups.filter(el => groups.includes(el.name)).map(el => el.students).flat().filter(el => el.name.includes(search)));
            return;
        }
        setStudents(FakeGroups.filter(el => groups.includes(el.name)).map(el => el.students).flat());
    }, [search]);
            ...
            <Table className={"!w-1/2 "}>
                <THead>
                    <TR className={"p-2 bg-[#A8A8A833] rounded "}>
                        <TH className={"flex justify-center gap-4 items-center"}>
                            <span>
                                اسم الطالب
                            </span>
                            <img src={sort}/>
                ...
                <TBody className={"mt-2 h-[380px] overflow-auto"}>
                    {students?.map((el, i) => <TR key={el.id}>
                        <TD>
