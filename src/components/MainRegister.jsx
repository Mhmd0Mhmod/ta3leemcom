...
export default function MainRegister() {
 ...
 return (
  <>
   <div
    className={
     "z-20 fixed w-full h-full top-0 left-0 bg-[rgb(0,0,0,0.4)] flex items-center justify-center font-cairo"
    }
    id={"bg"}
    onClick={(e) => {
     if (e.target.id === "bg") {
   ...
   >
    {/* Right Form */}
    <div
     className={"grid grid-cols-2 bg-white  w-1/2 rounded-3xl h-[45.908rem] "}
    >
     {formType === "login" && !loginType && (
      <div className={"p-9 flex flex-col font-almaria"}>
       ...
       <div className="ms-[1.875rem] flex flex-col gap-[1.875rem]">
        ...
        <p className="w-[15.625rem] text-[#A6A6A6]">
         يرجى اختيار نوع الحساب الذي ترغب في تسجيل الدخول به
        </p>
       </div>
       ...
       <div className="flex items-center content-center h-full  w-full">
        <div className="flex flex-col gap-20 m-auto w-[70%] ">
         <div>
          <Button
           className={
            "rounded w-full py-3 border border-[#0562CF] text-[#0462CF] hover:bg-[#0462CF] hover:text-white"
           }
         ...
         <div>
          <Button
           className={
            "w-full py-3 border border-[#0562CF] text-[#0462CF] hover:bg-[#0462CF] hover:text-white"
           }
           type={"outlineSecondary"}
     ...
     <div
      className={
       "bg-sign-up&login w-full h-full bg-cover bg-center rounded-3xl flex justify-center items-center relative "
      }
     >
      <div
       className={"flex flex-col text-white justify-around h-1/4 items-start  "}
      >
       <Heading as={"h1"}>تعليم كوم</Heading>
       <p>تجربة تعليمية ممتعة وسلسة لكافة مستخدمينا</p>
       <Button
        type={"normal"}
        className={"w-fit text-white bg-blue-600 border-0 text-sm rounded-3xl"}
       >
        اعرف المزيد
       </Button>
      </div>
